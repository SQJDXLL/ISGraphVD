...
def roc(all_scores, all_labels):
    
    scores_max = torch.max(all_scores)
    scores_min = torch.min(all_scores)

    # normalize scores to [0, 1] and add a small epislon for safety
    all_scores = (all_scores - scores_min) / (scores_max - scores_min + 1e-8)

    lall_labels = (all_labels + 1) / 2

    fpr, tpr, thresholds = metrics.roc_curve(all_labels.cpu().detach().numpy(), all_scores.cpu().detach().numpy())
