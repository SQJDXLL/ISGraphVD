...
class GraphEncoder(nn.Module):
    ...
    def forward(self, node_features, edge_features=None):
        ...
        # print(edge_features.shape, node_outputs.shape, self._edge_hidden_sizes)
...
def graph_prop_once(node_states,
    ...
    # print(to_idx[to_idx > node_states.shape[0]], node_states.shape)
    ...
    # print('in graph_prop_onece:')
    # print('the shape of node_states = {}'.format(list(node_states.size())))
    # print('the shape of from_states = {}'.format(list(from_states.size())))
    # print('the shape of to_states = {}'.format(list(to_states.size())))
    # print('the shape of edge_inputs = {}'.format(list(edge_inputs.size())))
...
class GraphPropLayer(nn.Module):
    ...
    def build_model(self):
        ...
        # ! +1 有问题
        # 这个地方不用改，因为使用了修改后的config
        # layer.append(nn.Linear(self._edge_hidden_sizes[0] + 1, self._edge_hidden_sizes[0]))
        layer.append(nn.Linear(self._edge_hidden_sizes[0], self._edge_hidden_sizes[0]))
        # print('in GraphPropLayer:')
        # print('edge_hidden_sizes = {}'.format(self._edge_hidden_sizes))
        # print('node_state_dim = {}'.format(self._node_state_dim))
        # print('node_hidden_sizes = {}'.format(self._node_hidden_sizes))
        # new所以是两层
        for i in range(1, len(self._edge_hidden_sizes)):
            layer.append(nn.ReLU())
            layer.append(nn.Linear(self._edge_hidden_sizes[i - 1], self._edge_hidden_sizes[i]))
        # print('in GraphPropLayer._message_net, num of layers = {}'.format(len(layer)))
    ...
    def _compute_aggregated_messages(
        ...
        """
        # print('in compute_aggregated_messages...')
        # print('node_state.size = {}'.format(list(node_states.size())))
        # print('from_idx.size = {}'.format(list(from_idx.size())))
        # print('to_dix.size = {}'.format(list(to_idx.size())))
        # print('edge_features.size = {}'.format(list(edge_features.size())))
        ...
        # print('the first graph_prop_once layer done!')
        # optionally compute message vectors in the reverse direction
        if self._use_reverse_direction:
    ...
    def _compute_node_update(self,
        ...
        if self._node_update_type == 'gru':
            ...
            # print('before input into gru:')
            # print('the shape of node_state_inputs = {}'.format(list(node_state_inputs.size())))
            # print('the shape of node_states = {}'.format(list(node_states.size())))
...
class GraphAggregator(nn.Module):
    ...
    def __init__(self,
                ...
                graph_transform_sizes=None,#128
                #  input_size=None,
                # ？？？如何初始化，
                 input_size=None,#56
                 gated=True,
    ...
    def forward(self, node_states, graph_idx, n_graphs):
        ...
        """
        # print("node_states", node_states.shape, node_states)
        ...
        if self._aggregation_type == 'max':
            ...
            # ? 0325
            # graph_states = graph_states * torch.FloatTensor(graph_states > -1e5)
...
class GraphEmbeddingNet(nn.Module):
    ...
    def _build_layer(self, layer_id):
        ...
        # print('in build_layer_{}'.format(layer_id))
        # print('layer_class = {}'.format(self._layer_class))
        # print('node_state_dim = {}'.format(self._node_state_dim))
        # print('edge_hidden_sizes = {}'.format(self._edge_hidden_sizes))
        # print('node_hidden_sizes = {}'.format(self._node_hidden_sizes))
        # print('edge_net_init_scale = {}'.format(self._edge_net_init_scale))
        # print('node_update_type = {}'.format(self._node_update_type))
        # print('prop_type = {}'.format(self._prop_type))
