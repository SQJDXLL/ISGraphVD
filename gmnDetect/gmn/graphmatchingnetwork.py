...
def topTenNodes(tensor):
    flattened_tensor = tensor.view(-1) 
    # 使用torch.topk获取前10个最大的数和它们的索引  
    top_values, top_indices = flattened_tensor.topk(50, dim=0) 
    for i in range(top_values.size(0)):  
      # 将一维索引转换回二维坐标  
      row_index = top_indices[i] // tensor.size(1)  
      col_index = top_indices[i] % tensor.size(1)  
      # print(f"Value: {top_values[i].item()}, Coordinates: ({row_index}, {col_index})")
...
def batch_block_pair_attention(data,
    ...
    for i in range(0, n_blocks, 2):
        ...
        # print("attention_x, attention_y",attention_x.shape, attention_y.shape, attention_x, attention_y)
...
class GraphMatchingNet(GraphEmbeddingNet):
    ...
    forms the first pair and graph 2 and 3 are the second pair etc., and computes
    cross-graph attention-based matching for each pair.
    # ! forward 方法调用的是 GraphEmbeddingNet
    """
