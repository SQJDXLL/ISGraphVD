...
if __name__ == "__main__":
    ...
    if hl:
        if utils.GRAPH_MODE == 'single':
            ...
            base_dir = os.path.join("../../realWorld/data/", project, cve_id, "matrix_{}_divide_hl".format(utils.GRAPH_MODE))
        else:
        ...
        if utils.GRAPH_MODE == 'single':
            ...
            base_dir = os.path.join("../../realWorld/data/", project, cve_id, "matrix_{}_divide".format(utils.GRAPH_MODE))
        else:
            from data.settings_disjoint import vul_dict
            base_dir = os.path.join("../../realWorld/data/", project, cve_id, "matrix_{}_divide".format(utils.GRAPH_MODE))
    # print("vul_dict", vul_dict)
    for proj, cve_dict in vul_dict.items():
        ...
        for cve in cve_dict.keys():
            ...
            if large:
                for adj in adjs:
                    adj = adj.strip()
                    node = adj.replace('_adj.npz', '_node.npz').replace('/adj/', '/node/')
                    item = {'adj': adj, 'node': node}
                    dataset[adj]=item
            else:
                for adj in adjs:
                    adj = adj.strip()
                    node = adj.replace('_adj.npy', '_node.npy').replace('/adj/', '/node/')
                    item = {'adj': adj, 'node': node}
                    dataset[adj]=item

            for item in dataset.keys():
                if detect_vul(model, project, cve_id, dataset[item]['adj'], dataset[item]['node']):
                    print("vul happen", item)
                else:
                    print("not vul happen", item)
