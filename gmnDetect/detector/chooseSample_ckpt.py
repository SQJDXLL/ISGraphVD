...
def get_input_data(adj_npy1, node_npy1, adj_npy2, node_npy2):
    if large:
...
def get_dissim(model, node_features, edge_features, from_idx, to_idx, graph_idx):
    ...
    for vul1 in dataset['vul']:
        try:
            ...
            for vul2 in dataset['vul']:
                ...
                data = get_input_data(vul1['adj'], vul1['node'], vul2['adj'], vul2['node'])
            
                if not model:
                    print(data[0].shape[-1], edge_feature_dim)
                    model = utils.load_model_ckpt(project, cve_id, device, bs, lr, epoch, hl, edge_feature_dim, data[0].shape[-1])

                if vul1 != vul2:
                    dis_sim = get_dissim(model, *data)
